...
// import placeOrder from '../../assets/image/place order-m.png';
...
const Order = () => {
  ...
  const columnName = [
      ...
      cellRenderer: (params) => {
        ...
        const imageLink = params?.data && params?.data['IMAGE LINK'] ? (
          <ImageIcon
            ...
            style={{ cursor: 'pointer', color: '#023020', width: '18px' }}
          />
        ) : <ImageNotSupportedIcon style={{ color: '#023020', width: '18px' }} />;
        ...
        const videoLink = params?.data && params?.data['VIDEO LINK'] ? (
          <VideoCameraBackIcon
            ...
            style={{ cursor: 'pointer', color: '#023020', width: '18px' }}
          />
        ) : <VideocamOffIcon style={{ color: '#023020', width: '18px' }} />;
        return (
    ...
    { field: 'Order_No', headerName: 'ORDER ID' },
    ...
    { field: 'PAV OPEN', headerName: 'PAVILION OPEN', width: '90' },
  ...
  const fetchCartData = () => {
    ...
    ApiPost(`${EndPoint?.GET_ORDER}?stock_Id=${stockidData}&from_Date=${formatDate(selectedFromDate)}&to_Date=${formatDate(selectedToDate)}`)
      .then((res) => {
        if (res?.status === 200) {
  ...
  const getAllLayout = async (test) => {
    await ApiGet(`${EndPoint.GET_ALL_REPORT}?rm_Id=4`)
      .then((res) => {
        if (res?.status === 200) {
  ...
  const ExcelDownloadBtn = () => {
    ...
    promises.push(ApiPost(`${EndPoint?.EXCEL_DOWNLOAD_ORDER}`, body)
      .then(async (res) => {
  ...
  const sendToEmailData = () => {
    ...
    ApiPost(`${EndPoint?.ORDER_SEND_EMAIL}`, body)
      .then((res) => {
        if (res?.data?.statusCode === 200) {
          setRemark('');
          setSendMail('');
  ...
  const clickOnSkypeImg = async (id) => {
    ...
    if (checkedItems?.certificate
      ...
      if (checkedItems?.customerExcel) {
        ...
        try {
          const res = await ApiPost(`${EndPoint.EXCEL_DOWNLOAD_ORDER}`, body);
          if (res.status === 200) {
            const fileUrl = res?.data?.result;
            if (fileUrl) {
